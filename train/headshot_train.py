...
def add_arg(name, value, is_flag=False, ignore_if_empty_str=True, positive_check=False):
    if is_flag:
        if value: train_args.append(name)
    elif positive_check:
        try:
            if value is not None and float(value) > 0: train_args.append(f"{name}={value}")
        except (ValueError, TypeError): pass
    elif value is not None:
        if isinstance(value, str) and ignore_if_empty_str and not value: return
...
current_epoch = config.get("epoch", 0) # epoch from JSON maps to max_train_epochs effectively
...
if current_max_train_steps > 0:
    ...
    if current_lr_warmup_steps == 0 and current_lr_warmup_ratio_perc > 0:
        warmup_as_steps = int(current_max_train_steps * (current_lr_warmup_ratio_perc / 100))
        if warmup_as_steps > 0: add_arg("--lr_warmup_steps", warmup_as_steps)
    ...
    if current_lr_warmup_steps == 0 and current_lr_warmup_ratio_perc > 0:
        ...
        if warmup_ratio > 0: add_arg("--lr_warmup_ratio", warmup_ratio)
    elif current_lr_warmup_steps > 0:
        add_arg("--lr_warmup_steps", current_lr_warmup_steps)

add_arg("--lr_scheduler_args", config.get("lr_scheduler_args", ""))
if config.get("lr_scheduler_num_cycles",0) > 0 and config.get("lr_scheduler") in ["cosine_with_restarts", "polynomial_with_restarts", "cosine"]:
     add_arg("--lr_scheduler_num_cycles", config["lr_scheduler_num_cycles"])
if config.get("lr_scheduler_power",1) != 1 and config.get("lr_scheduler") == "polynomial":
     add_arg("--lr_scheduler_power", config["lr_scheduler_power"])
optimizer_args_str = config.get("optimizer_args", "")
if optimizer_args_str: train_args.extend(optimizer_args_str.split())

# Network (LoRA) settings
if config.get("LoRA_type") == "Standard": add_arg("--network_module", "networks.lora")
add_arg("--network_dim", config["network_dim"])
add_arg("--network_alpha", config["network_alpha"])

lora_network_args_list = []
if config.get("block_dims"): lora_network_args_list.append(f"network_block_dims={config.get('block_dims')}")
if config.get("block_alphas"): lora_network_args_list.append(f"network_block_alphas={config.get('block_alphas')}")
if config.get("block_lr_zero_threshold"): lora_network_args_list.append(f"block_lr_zero_threshold={config.get('block_lr_zero_threshold')}")
if config.get("down_lr_weight"): lora_network_args_list.append(f"down_lr_weight={config.get('down_lr_weight')}")
if config.get("mid_lr_weight"): lora_network_args_list.append(f"mid_lr_weight={config.get('mid_lr_weight')}")
if config.get("up_lr_weight"): lora_network_args_list.append(f"up_lr_weight={config.get('up_lr_weight')}")
if config.get("additional_parameters"): lora_network_args_list.append(config.get("additional_parameters")) # For raw passthrough
if lora_network_args_list: train_args.append(f"--network_args {' '.join(lora_network_args_list)}")
...
add_arg("--save_every_n_epochs", config.get("save_every_n_epochs", 0), positive_check=True)
add_arg("--save_every_n_steps", config.get("save_every_n_steps", 0), positive_check=True)

# Other Training Options
add_arg("--gradient_checkpointing", config.get("gradient_checkpointing", False), is_flag=True)
add_arg("--cache_latents", config.get("cache_latents", False), is_flag=True)
if config.get("cache_latents", False):
    add_arg("--cache_latents_to_disk", config.get("cache_latents_to_disk", False), is_flag=True)
...
if config.get("xformers") == "xformers": add_arg("--xformers", True, is_flag=True)
elif config.get("xformers") == "sdpa": add_arg("--sdpa", True, is_flag=True)

add_arg("--persistent_data_loader_workers", config.get("persistent_data_loader_workers", False), is_flag=True)
...
add_arg("--caption_dropout_every_n_epochs", config.get("caption_dropout_every_n_epochs", 0), positive_check=True)
add_arg("--caption_dropout_rate", config.get("caption_dropout_rate", 0), positive_check=True)
...
add_arg("--debiased_estimation_loss", config.get("debiased_estimation_loss", False), is_flag=True)
...
if config.get("sample_every_n_steps", 0) > 0 or config.get("sample_every_n_epochs", 0) > 0:
    ...
    if prompts_str:
        with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".txt", encoding='utf-8') as tmpfile:
            prompts_cleaned = [p.strip() for p_line in prompts_str.split(',') for p in p_line.split(';') if p.strip()]
    ...
    add_arg("--sample_every_n_steps", config.get("sample_every_n_steps", 0), positive_check=True)
    add_arg("--sample_every_n_epochs", config.get("sample_every_n_epochs", 0), positive_check=True)
...
add_arg("--stop_text_encoder_training", config.get("stop_text_encoder_training", 0), positive_check=True)
add_arg("--train_norm", config.get("train_norm", False), is_flag=True)
add_arg("--scale_weight_norms", config.get("scale_weight_norms", 0), positive_check=True)

if config.get("save_state", False): add_arg("--save_state", True, is_flag=True)
...
try:
    ...
    process = subprocess.Popen(final_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', bufsize=1)
    while True:
        output = process.stdout.readline()
        if output == '' and process.poll() is not None: break
        if output: print(output.strip())
    rc = process.poll()
    if rc == 0: print(f"✅ Training completed successfully.")
    else: print(f"❌ Training process exited with an error code: {rc}")
